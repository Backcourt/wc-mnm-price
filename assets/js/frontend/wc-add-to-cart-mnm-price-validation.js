	...
	function WC_MNM_Price( container ) {
		...
		this.validate = function ( event, container ) {
			...
			if ( min_price === max_price && total_price !== min_price ) {
				error_message = wc_mnm_price_params.i18n_price_error.replace(
					'%s',
					wc_mnm_price_format( min_price )
				);
			}
			// Validate a range.
			else if (
				max_price > 0 &&
				min_price > 0 &&
				( total_price < min_price || total_price > max_price )
			) {
				error_message = wc_mnm_price_params.i18n_min_max_price_error
					.replace( '%max', wc_mnm_price_format( max_price ) )
					.replace( '%min', wc_mnm_price_format( min_price ) );
			}
			// Validate that a container has minimum price.
			else if ( min_price > 0 && total_price < min_price ) {
				error_message =
					wc_mnm_price_params.i18n_min_price_error.replace(
						'%min',
						wc_mnm_price_format( min_price )
					);
				...
				error_message =
					wc_mnm_price_params.i18n_max_price_error.replace(
						'%max',
						wc_mnm_price_format( max_price )
					);
			}
			...
			if ( error_message !== '' ) {
				// "Selected Xunit".
				var selected_price_message =
					self.selected_price_message( total_price );

				// Add error message, replacing placeholders with current values.
				container.add_message(
					error_message.replace( '%v', selected_price_message ),
					'error'
				);
		...
		this.selected_price_message = function ( price ) {
			return wc_mnm_price_params.i18n_price_message.replace(
				'%s',
				wc_mnm_price_format( price )
			);
		};
	} // End WC_MNM_Price.
